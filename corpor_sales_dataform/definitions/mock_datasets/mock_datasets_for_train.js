publish('store_item_class_mock_2017614', { type: "table", tags: ["merge_test"] }).query(ctx => `
  SELECT * FROM UNNEST([
...
publish("store_item_rolling_stats_mock_2017614", { type: "table", tags: ["merge_test"] }).query(ctx => `
  SELECT * FROM UNNEST([
...
publish("item_promo_first_last_app_mock_2017614", { type: "table", tags: ["merge_test"] }).query(ctx => `
  SELECT * FROM UNNEST([
...
publish("class_store_has_promo_sales_decay_sum_mock_2017614", { type: "table", tags: ["merge_test"] }).query(ctx => `
  SELECT * FROM UNNEST([
